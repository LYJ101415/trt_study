...
def postprocess_yolo(pred, img_info, conf_thres=0.001, iou_thres=0.65, max_det=300):
    ...
    # 注意: cv2.dnn.NMSBoxes 按 C++ Rect(x,y,w,h) 语义解析输入框，须传宽高；
    # 直传 xyxy 会把 x2/y2 当宽高，IoU 全错、多框时 NMS 结果错误。
    # boxes_xyxy 保持 xyxy 不动（后面组装检测结果还要用），单独转一份 w/h 供 NMS。
    boxes_for_nms = boxes_xyxy.copy()
    boxes_for_nms[:, 2] -= boxes_for_nms[:, 0]
    boxes_for_nms[:, 3] -= boxes_for_nms[:, 1]
    indices = cv2.dnn.NMSBoxes(boxes_for_nms.tolist(), max_scores.tolist(),
