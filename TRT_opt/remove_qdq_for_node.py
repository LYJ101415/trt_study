...
def remove_chains_and_reconnect(model, chains, consumer_node_name, verbose=False):
    """
    根据找到的 Q/DQ 链执行删除和重连.
    原地修改 model.
    ...
    # 收集所有需要删除的节点名 (去重)
    nodes_to_remove = set()
    reconnect_map = {}  # dq_output → upstream_tensor
...
def main():
    ...
    parser.add_argument("--model", required=True, help="输入 INT8 ONNX 路径")
    parser.add_argument("--node", required=True, help="目标算子节点名, 如 Conv_250")
    parser.add_argument("--output", "--out", required=True, help="输出模型路径")
