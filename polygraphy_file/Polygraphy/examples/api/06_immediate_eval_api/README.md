...
5. Run the script that loads the previously built engine, then runs it:
    ...
    ```
==============================================================
这份文档标题为 “Immediately Evaluated Functional API”，是 NVIDIA Polygraphy 工具包中关于 “立即求值函数式 API” 的核心说明。其目标是解决 懒加载（Lazy Loader）API 在某些场景下可读性差、易混淆 的问题，提供一套更直观、更符合 Python 习惯的同步执行接口。
